...
class Command(BaseCommand):
    help = "Seeds the database with initial Kenyan supermarkets, products, and prices for testing."
    ...
    def handle(self, *args, **kwargs):
        ...
        # Prices at Carrefour Sarit Centre
        ...
        # Prices at Naivas Westlands
        ProductPrice.objects.create(product=milk, branch=nv_westlands, price=Decimal("98.00"))  # Cheaper milk
        ProductPrice.objects.create(product=sugar, branch=nv_westlands, price=Decimal("215.00"))
        ProductPrice.objects.create(product=maize, branch=nv_westlands, price=Decimal("180.00")) # Cheaper maize
        ProductPrice.objects.create(product=bread, branch=nv_westlands, price=Decimal("62.00"))  # Cheaper bread
        ProductPrice.objects.create(product=cooking_oil, branch=nv_westlands, price=Decimal("350.00"))

        # Prices at Naivas Kilimani (missing cooking oil)
        ...
        # Prices at Quickmart Lavington (missing sugar)
        ProductPrice.objects.create(product=milk, branch=qm_lavington, price=Decimal("105.00"))
        ProductPrice.objects.create(product=maize, branch=qm_lavington, price=Decimal("178.00")) # Cheaper maize
        ProductPrice.objects.create(product=bread, branch=qm_lavington, price=Decimal("60.00"))  # Cheaper bread
        ProductPrice.objects.create(product=cooking_oil, branch=qm_lavington, price=Decimal("335.00")) # Cheaper oil

        self.stdout.write(self.style.SUCCESS("Database seeded successfully with Kenyan supermarket data!"))
